...
export default function decorate(block) {
  ...
  rows.forEach((row, idx) => {
    ...
    // LCP: the first slide is the hero image — load it eagerly at high priority.
    // Hidden slides load lazily at low priority so they don't contend for bandwidth.
    const slideImg = slide.querySelector('.carousel-hero-slide-image img');
    if (slideImg) {
      if (idx === 0) {
